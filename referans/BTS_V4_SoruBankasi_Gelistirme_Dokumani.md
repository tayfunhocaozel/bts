...
> - **MD dışa aktarma — eklendi:** `yanlis_defteri.soru_metni` (TEXT) kolonu; `soru-analiz` edge function (v2) fotoğraftan sorunun tam metnini + şıklarını çıkarıp bu alana yazar. Öğretmen aynı checkbox seçimiyle **"MD Oluştur"** der → konuya göre gruplu, kazanım + doğru cevap + soru metni + foto bağlantısı içeren `.md` dosyası indirilir (Blob + `<a download>`). Amaç: dosyayı harici soru-yazma uygulamasına verip öğrenciye özel ikiz sorular üretmek. Metni olmayan eski kayıtlar için seçim çubuğunda **"Metni Çıkar (K)"** — seçili sorularda `soru-analiz`'i tekrar çağırıp yalnızca `soru_metni`'yi günceller (konu/kazanım/onay durumu değişmez).
